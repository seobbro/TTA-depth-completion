...
def robustness_loss_func(src, tgt, w=None):
    '''
...
'''
def patchfy(images, box_occlusion_size, rand_range):
    '''
    output:
    '''

    # Image input should be Tensor with size N C H W
    assert(len(images.size())==4)

    C, H, W = images[0].size()
    crop_h, crop_w = box_occlusion_size
    rand_range_h, rand_range_w = rand_range

    mask_list = []
    for image in images:
        randh, randw = torch.randint(0, rand_range_h, (1,)), torch.randint(0, rand_range_w, (1,))
        mask = torch.zeros_like(image)
        mask[:, randh: randh + crop_h, randw: randw + crop_w] = 1
        mask_list.append(mask.view(-1, C, H, W))
        # wtl2Matrix = image.clone().detach()
        # wtl2Matrix.data.fill_(0.98 * 1)
        # wtl2Matrix.data[:, randh: randh + crop_h, randw: randw + crop_w] = 0.98
        # weight_list.append(wtl2Matrix.view(-1, C,H,W))
    mask = torch.cat(mask_list, dim=0)
    # wtl2Matrix = torch.cat(weight_list, dim=0)

    return mask


def dist_aware_patchfy(image, sparse_depth, validation_map, crops):
    crop_h, crop_w = crops['crop_hw'][0], crops['crop_hw'][1]
    r_mean, g_mean, b_mean = 0.485, 0.456, 0.406  # imagenet mean rgb values.
    rand_range_h, rand_range_w = crops['rand_range'][0], crops['rand_range'][1]

    randh, randw = torch.randint(0, rand_range_h, (1,)), torch.randint(0, rand_range_w, (1,))
    input_real_data = image
    input_cropped_data = image.clone()
    input_cropped_data[:, 0, randh: randh + crop_h,
    randw: randw + crop_w] = r_mean  # G mean of training set
    input_cropped_data[:, 1, randh: randh + crop_h,
    randw: randw + crop_w] = g_mean  # G mean of training set
    input_cropped_data[:, 2, randh: randh + crop_h,
    randw: randw + crop_w] = b_mean  # B mean of training set

    wtl2Matrix = image.clone().detach()
    wtl2Matrix.data.fill_(0.98 * 10)
    wtl2Matrix.data[:, 0, randh: randh + crop_h,
    randw: randw + crop_w] = 0.98

    patches = {'real': input_real_data, 'crop': input_cropped_data, 'weight': wtl2Matrix}
    return patches


def compute_nlspn_loss(max_predict_depth, output, ground_truth_depth,
                       patches=None, crops=None, validity_map=None, perceptual=False, style=False,
                       l1_weight=1.0,
                 l2_weight=1.0):
    '''
    ...
    Arg(s):
        ...
        validity_map : torch.Tensor[float32]
            N x 1 x H x W ground truth validity map
        l1_weight : float
            weight of l1 loss
        l2_weight : float
            weight of l2 loss
    ...
    output_depth = torch.clamp(output_depth, min=0, max=max_predict_depth)
    ground_truth_depth = torch.clamp(ground_truth_depth, min=0, max=max_predict_depth)

    # Obtain valid values
    ground_truth = ground_truth_depth

    # Compute individual losses
    l1_loss_dep = l1_loss(ground_truth, output_depth, validity_map)
    l2_loss_dep = l2_loss(ground_truth, output_depth, validity_map)
    loss = l1_weight * l1_loss_dep + l2_weight * l2_loss_dep
...
def compute_nlspn_loss_validation(args, output_depth,
                 ground_truth_depth,
                 l1_weight=1.0,
                 l2_weight=1.0):
    '''
    ...
    validity_map = torch.from_numpy(ground_truth_depth[:, :, 1])
    ground_truth = torch.from_numpy(ground_truth_depth[:, :, 0])
    output_depth = torch.from_numpy(output_depth)

    output_depth = torch.clamp(output_depth, min=0, max=args.max_predict_depth).unsqueeze(0).unsqueeze(0)
    ground_truth = torch.clamp(ground_truth, min=0, max=args.max_predict_depth).unsqueeze(0).unsqueeze(0)
    validity_map = validity_map.unsqueeze(0).unsqueeze(0)
...
def preadapt_loss(max_predict_depth, output, ground_truth_depth,
                 patches, crops,validity_map,
                 l1_weight=1.0,
                 l2_weight=1.0,
                inpaint_weight = 0.5,
                perceptual = False,
                ...
                percept_weight = 0.1):
    # Inpainting loss
    output_depth, image_fake = output['pred'], output['fake']
    image_real, wtl2Matrix = patches['real'], patches['weight']
    b, c, h, w = image_real.size()
    # loss_G_l2 = l2_loss(image_fake, image_real, wtl2Matrix)
    loss_G_l1 = l1_loss(image_fake, image_real, wtl2Matrix)
    loss_G = inpaint_weight * loss_G_l1


    if perceptual:
        perceptual_loss = 0.0
        encoder_real = output['perceptual'][1]
        encoder_cropped = output['perceptual'][0]
        for (cropped_act, real_act) in zip(encoder_cropped, encoder_real):
            perceptual_loss += l1_loss(cropped_act, real_act)
        loss_G = inpaint_weight * loss_G_l1 + percept_weight * perceptual_loss
    if style:
        feat_fake = image_fake.view(b * c, h * w)
        feat_real = image_real.view(b * c, h * w)
        gram_fake = torch.mm(feat_fake, feat_fake.t()).div(b*c*h*w)
        gram_real = torch.mm(feat_real, feat_real.t()).div(b*c*h*w)
        style_loss = F.mse_loss(gram_fake, gram_real).abs()
        loss_G = loss_G + 250 * style_loss
    ...
    l1_loss_dep = l1_loss(ground_truth, output_depth, validity_map)
    l2_loss_dep = l2_loss(ground_truth, output_depth, validity_map)
    loss = l1_weight * l1_loss_dep + l2_weight * l2_loss_dep + loss_G
    ...
    if perceptual:
        loss_info['perceptual'] = perceptual_loss
    if style:
        loss_info['style'] = style_loss
    return loss, loss_info


def adapt_loss(input_rgb, output_depth, sparse_depth, validity_map):



    loss_smooth = smoothness_loss_func(output_depth, input_rgb)
    loss_sparse_consistency = sparse_depth_consistency_loss_func(output_depth, sparse_depth, validity_map)
    loss =loss_sparse_consistency  +  loss_smooth
    loss_info = {'loss': loss,'smooth': loss_smooth}#, 'sparse': loss_sparse_consistency}

    return loss, loss_info

def tta_loss(args, output, sparse_depth,
                 patches, crops, validity_map, perceptual = False,
             style=False,
             l1_weight=1.0,
             l2_weight=1.0):
    ...
    overlap_h, overlap_w = crops['offset']
    patch_h, patch_w = image_real.size(2), image_real.size(3)

    loss_G_l2 = (image_fake - image_real).pow(2)
    loss_G_l1 = (image_fake - image_real).abs()
    loss_G_l2 = loss_G_l2 * wtl2Matrix
    loss_G_l2 = loss_G_l2.mean()
    loss_G_l1 = loss_G_l1 * wtl2Matrix
    loss_G_l1 = loss_G_l1.mean()

    loss_G = loss_G_l2 + loss_G_l1
    ...
    '''
            ...
            '''
    # They clamp their predictions
    output_depth = torch.clamp(output_depth, min=0, max=args.max_predict_depth)
    sparse_depth = torch.clamp(sparse_depth, min=0, max=args.max_predict_depth)
    # validity_map_depth = torch.where(sparse_depth > 0, torch.ones_like(sparse_depth), sparse_depth)
    loss_sparse_depth = sparse_depth_consistency_loss_func(output_depth, sparse_depth, validity_map)
    loss_smoothness = smoothness_loss_func(predict=output_depth, image=image_real)
    ...
    loss = loss_G + loss_sparse_depth  # + loss_smoothness

    # Store loss info
    loss_info = {
        'loss_smooth': loss_sparse_depth,
        # 'loss_consistency': loss_smoothness,
        'loss_G': loss_G,
        'loss': loss
    }
    return loss, loss_info
