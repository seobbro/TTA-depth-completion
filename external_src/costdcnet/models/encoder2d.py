...
class Encoder2D(nn.Module):
    def __init__(self, in_ch = 3,output_dim=128):
        ...
        self.meta_bn_rgb = None
        self.conv1_rgb_meta = None
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        if self.meta_bn_rgb is not None and self.conv1_rgb_meta is not None:
            x_meta = self.conv1_rgb_meta(x)
            x = self.meta_bn_rgb(self.conv2(x))
            x += x_meta
        else:
            x = self.conv2(x)
